<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="ninebox_single.aspx.cs" Inherits="ninebox_single" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="pageheader" runat="Server">
    ...
    <div class="pageheader">
        ...
        <div class="pagetitle">
            <h5>Department Level Scores acheived by Employees on Performance Tests and Competency</h5>
            <h1>Nine Box One</h1>
        </div>
    </div>
    <!--pageheader-->
                ...
                <div class="col-md-6" runat="server">
                    <div class="box box-primary" runat="server">
                        <div class="box-header with-border" runat="server">
                            <h3 class="box-title" runat="server">
                                <label id="lblEmpID"><%#Eval("Department") %></label>&nbsp; Department</h3>
                            </a>
                        ...
                        <div class="box-body" style="height: 300px; width: 600px" runat="server">
                            <div class="chart-container" runat="server">
                                <canvas id="mgrChart<%#Eval("Department").ToString().Replace(" ","_") %>" class="mgrChart<%#Eval("Department").ToString().Replace(" ","_") %>" style="height: 300px; width: 600px"></canvas>
                            </div>
                        </div>
                    </div>
                    <!-- /.box -->
...
<asp:Content ID="Content6" ContentPlaceHolderID="below_footer" runat="Server">
    ...
    <script type="text/javascript">
        //debugger;
        var Period = $("#ddlPeriod").children("option:selected").val();
        $(function () {
            updateAllCharts();
        });
        ...
        function updateAllCharts() {
            ...
            $('[class*="mgrChart"]').each(function () {
                var Dept = $(this).prop('id');
                Dept = Dept.replace("mgrChart", "").replace("_", " ");
                if (Dept.length > 0) {
                    fillChart(Dept, Period);
                }
            });
        }
        function fillChart(Dept, Period) {
            var params = '{"Dept":"' + Dept + '","Period":"' + Period + '"}';
            if (Dept != "" && Dept != "0") {

                $.ajax({
                    type: "POST",
                    url: "ninebox_one.aspx/GetBubbleChart_ByDesignation",
                    data: params,
                    async: true,
            ...
            function CreateNineBoxChart(xdata) {
                var ChartAtDept = Dept.replace(" ", "_");
                var ctx = $("#mgrChart" + ChartAtDept);
                if (xdata != null) {
                    ctx.empty();
                }
                var myChart = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        title: {
                            display: false,
                            text: 'Scores acheived by Employees on Performance Tests and Competency'
                        },
                        legend: {
                            display: false,
                            position: 'right'
